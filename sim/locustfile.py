...
SIM_TASKS_PER_USER_PER_SEC (1), SIM_NEWCOMERS_PER_SEC (0.2). Accounts are
@sim.invalid users - run only against a disposable database.
"""
...
from locust import HttpUser, constant_throughput, task  # noqa: E402

from sim.driver import ApiClient  # noqa: E402
from sim.load import CLICK_WEIGHT, TODAY_WEIGHT, LoadUser, UserPool, swallow_api_errors  # noqa: E402
...
class ActiveReader(HttpUser):
    ...
    def on_start(self):
        self.load = LoadUser(READERS.take(), ApiClient(self.client, locust=True), seed=SEED)
...
class Newcomer(HttpUser):
    ...
    def signup_onboard_first_view(self):
        load = LoadUser(NEWCOMERS.take(), ApiClient(self.client, locust=True), seed=SEED)
