...
achieved RPS, p50/p95/p99 (ms), from Locust's `<prefix>_stats.csv`.
"""
...
def run_stage(host: str, rps: int, duration: str, prefix: Path, env: dict) -> Path:
...
def main(argv=None) -> int:
    ...
    stages = {}
    for rps in args.rps:
        stats = run_stage(args.host, rps, args.duration, args.out_dir / f"rps{rps}", env)
        stages[str(rps)] = summarize_locust_csv(stats)
    report = {"host": args.host, "duration": args.duration, "stages": stages}
