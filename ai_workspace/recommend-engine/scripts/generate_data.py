...
# [설정] 표준 카테고리 및 매핑 규칙 정의
STANDARD_CATEGORIES = ["정치", "경제", "사회", "생활문화", "IT과학", "세계", "스포츠"]
CATEGORY_MAP = {
    "문화": "생활문화",
    "국제": "세계"
}
...
def generate_data():
    ...
    target_user_count = gen_config.get('num_dummy_users', 30)
    
    # 2. 원본 데이터 로드
    local_data_path = os.path.join(project_root, "data", "newsletter_share.json")
    if not os.path.exists(local_data_path):
        shared_path = os.path.join(project_root, "../../newsletter_share.json")
    ...
    # 3. 데이터 구조 탐색
    items_iterator = find_news_items(raw_data)
    ...
    for item in items_iterator:
        ...
        final_id = str(raw_id) if raw_id is not None else str(len(news_list) + 1)

        # 카테고리 전처리 (표준화 적용)
        raw_cat = item.get('categories') or item.get('category', '')
        normalized_cats = normalize_categories(raw_cat)
        category_str = ", ".join(normalized_cats)
    ...
    embedder = BGEEmbeddingModel(config['model'])
    
    all_texts = [n['full_text'] for n in news_list]
    all_ids = [n['id'] for n in news_list]
    ...
    for i in range(1, target_user_count + 1):
        ...
        for n in news_list:
            ...
            if not news_cats_set.isdisjoint(user_prefs_set): # 교집합이 있으면 True
                candidate_news.append(n)
        
        # 3. 후보군 부족 시 재시도 (현실성 보정)
        retry = 0
        ...
        # 4. 뉴스 선택
        if candidate_news:
            sample_size = min(len(candidate_news), random.randint(3, 5))
            selected_news = random.sample(candidate_news, k=sample_size)
            selected_ids = [n['id'] for n in selected_news]
        else:
            selected_ids = [] # Cold Start
