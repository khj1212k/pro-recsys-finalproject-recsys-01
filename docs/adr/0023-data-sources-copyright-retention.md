...
- 그래서 그 브랜치가 병합된 뒤 다음을 넣는다. 트리거는 수집 런타임 PR(#8)의 `main` 병합이다. 병합하는 쪽은 이 TODO를 후속 작업으로 등록한다.

1. 마이그레이션(`down_revision` = `d48994e9d26e`, 즉 PR #8 병합 시점의 head. 그 사이 다른 마이그레이션이 먼저 들어오면 그때의 head)
